...
#[derive(Debug, Clone)]
pub struct Varargs<'s, 'g>(std::marker::PhantomData<&'s ()>, std::marker::PhantomData<&'g ()>);
...
#[derive(Debug, Clone)]
pub struct FunctionRef<'s, 'g>(std::marker::PhantomData<&'s ()>, std::marker::PhantomData<&'g ()>);
...
impl<'s, 'g> FunctionRef<'s, 'g> {
    fn make(f: impl Fn(FunctionInput<'s, 'g>) -> Vec<Value<'s, 'g>> + 'g, input: FunctionInput<'s, 'g>) -> FunctionRef<'s, 'g> {
        // TODO: Implement
...
impl<'s, 'g> String<'s, 'g> {
    ...
    pub fn to_number(&self) -> Option<Number> {
        match &self.0 {
            StringEnum::StaticString(s) => {
                match s.get_str().parse::<Number>() {
                    Ok(n) => Some(n),
                    Err(_) => None,
                }
            }
            StringEnum::DynamicString(s) => {
                s.to_number()
            }
        }
    }
    ...
    pub fn concat(l: String<'s, 'g>, r: String<'s, 'g>) -> String<'s, 'g> {
        match (l.0, r.0) {
            (StringEnum::StaticString(s1), StringEnum::StaticString(s2)) => String(StringEnum::DynamicString(StringRef::concat_ss(s1.get_str(), s2.get_str()))),
            (StringEnum::StaticString(s1), StringEnum::DynamicString(s2)) => String(StringEnum::DynamicString(StringRef::concat_sd(s1.get_str(), s2))),
            (StringEnum::DynamicString(s1), StringEnum::StaticString(s2)) => String(StringEnum::DynamicString(StringRef::concat_ds(s1, s2.get_str()))),
            (StringEnum::DynamicString(s1), StringEnum::DynamicString(s2)) => String(StringEnum::DynamicString(StringRef::concat_dd(s1, s2))),
        }
    }
