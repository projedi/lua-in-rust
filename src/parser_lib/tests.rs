...
fn run_parser<'a, T>(
    ...
    super::string::run_parser(input, p)
}
...
fn test_fail() {
    assert_eq!(
        run_parser("", fail_parser::<std::str::Chars, ()>()),
        (None, "")
    );
    assert_eq!(
...
fn test_map_filter() {
    ...
    assert_eq!(
        run_parser("", map_filter(none_f, empty_parser())),
        (None, "")
    );
    assert_eq!(
    ...
    assert_eq!(
        run_parser("abc", map_filter(none_f, empty_parser())),
        (None, "abc")
    );
...
fn test_choices0() {
    assert_eq!(
        run_parser("", choices::<std::str::Chars, ()>(vec![])),
        (None, "")
    );
    assert_eq!(
...
fn test_choices1() {
    ...
    assert_eq!(
        run_parser("bac", choices(vec![char_parser('a')])),
        (None, "bac")
    );
}
...
fn test_not_parser() {
    ...
    assert_eq!(
        run_parser("abc", not_parser(char_parser('a'))),
        (None, "abc")
    );
}
...
fn test_string_parser() {
    ...
    assert_eq!(run_parser("abd", string_parser("abc")), (None, "abd"));
...
fn test_eof() {
    ...
    assert_eq!(run_parser("a", eof()), (None, "a"));
    assert_eq!(
...
fn test_mut_recursive_parser() {
    ...
    assert_eq!(run_parser("130", mut_rec_parser1()), (None, "130"));
    assert_eq!(run_parser("1230", mut_rec_parser1()), (None, "1230"));
    ...
    assert_eq!(run_parser("230", mut_rec_parser2()), (None, "230"));
    assert_eq!(run_parser("2130", mut_rec_parser2()), (None, "2130"));
}
